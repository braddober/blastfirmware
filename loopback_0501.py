...
class roachInterface(object):
	...
	def __init__(self):
		...
		self.bitstream = './blastfirmware0425.fpg'
		...
		if (self.fpga.is_connected() == True):
			print 'Connection established, uploading bitstream...'
		...
		#self.freqs = np.array([152.,188.234, 200.])*1.0e6 
		self.light_rf_freqs = np.load('light_kids.npy')
		...
		self.test_comb = np.linspace(-200.0e6, 200.0e6, 1000)
		...
		self.main_opts= ['Initialize','Write Test Tone','Write DAC, DDS LUTs','Stream UDP packets','VNA sweep and plot','Locate resonances','Target sweep and plot','Bind Socket', 'Exit'] 
		"""	
	...
	# Generates a frequency comb for the DAC or DDS look-up-tables. DAC_LUT = True for the DAC LUT. Returns I and Q 
		...
		if DAC_LUT:
			...
			#I = (wave.real) *1.0e5*amp_full_scale
			#Q = (wave.imag) *1.0e5* amp_full_scale
		else:
...
# Builds the DDS look-up-table from I and Q given by freq_comb. freq_comb is called with the sample rate equal to the sample rate for a single FFT bin. There are two bins returned for every fpga clock, so the bin sample rate is 256 MHz / half the fft length  
	...
	def read_accum_snap(self):
		...
		#self.fpga.write_int('gbe_data_snap_ctrl', 0)
        	#self.fpga.write_int('gbe_data_snap_ctrl', 1)
        	#accum_data = np.fromstring(self.fpga.read('gbe_data_snap_bram', 2**13), dtype = '>i').astype('float')
	...
	def target_sweep(self, save_path = '/mnt/iqstream/target_sweeps', write = True, span = 100.0e3):
		...
		self.sweep_lo(Npackets_per = 1000, channels = channels, center_freq = center_freq, span = span , save_path = save_path)
	...
	def plot_kids(self, save_path = None, bb_freqs = None, channels = None):
		...
		[ plt.plot((lo_freqs + bb_freqs[chan])/1.0e9,mags[chan]) for chan in range(len(channels))]
	...
	def main_opt(self):
		while True:
			...
			if opt == 0:
				...
				self.initialize() 
			if opt == 1:
				print '\nTest tone (MHz) =', self.test_comb/1.0000e6
				self.writeQDR(self.test_comb)
				self.fpga.write_int('sync_accum_reset', 0)
				self.fpga.write_int('sync_accum_reset', 1)
			if opt == 2:
				print '\nDAC freqs (MHz) =', self.freqs/1.00000e6
