...
class roachInterface(object):
	...
	def __init__(self):
		...
		self.freqs = np.linspace(1,500,1024)*1e6
		self.freqs = np.load('/mnt/iqstream/last_bb_freqs.npy')
		...
		self.main_opts= ['Calibrate QDR','Initialize GbE (Must toggle before writing first tone)','Write Test Tone','Write DAC, DDS LUTs','Stream UDP packets','VNA sweep and plot','Locate resonances','Target sweep and plot','Exit'] 
		#sys.stdout.flush()
		self.dest_ip  = 192*(2**24) + 168*(2**16) + 41*(2**8) + 2 
		self.dest_port= 60001 
		...
		self.UDP_IP = "192.168.41.2" # local ip
		self.UDP_PORT = 60001 # local port
		...
		self.bitstream = '../blastfirmware_nist/blast_0120_ppstest_2016_Jan_25_1529.fpg'
	...
	def freq_comb(self, freqs, samp_freq, resolution,phase = np.array([0.]*1024), random_phase = True, DAC_LUT = True):
...
# Builds the DDS look-up-table from I and Q given by freq_comb. freq_comb is called with the sample rate equal to the sample rate for a single FFT bin. There are two bins returned for every fpga clock, so the bin sample rate is 256 MHz / half the fft length  
	...
	# packs the I and Q look-up-tables into strings of 16-b integers, in preparation to write to the QDR. Returns the string-packed look-up-tables
		self.I_dac, self.Q_dac = self.freq_comb(freqs, self.dac_samp_freq, self.dac_freq_res)
	...
	def generate_full_scale_white_noise_file(self):
		...
		wn *= 2**15-1
		wn -= 2**14
		np.savetxt('white_noise.txt',np.round(wn).astype(int),fmt='%d')
		return
	
	def writeQDR_white_noise_full_scale(self):
		wn = np.loadtxt('white_noise.txt')
		freqs=self.freqs
		self.I_dac, self.Q_dac = wn,wn
		self.I_dds, self.Q_dds = self.define_DDS_LUT(freqs)
		self.I_lut, self.Q_lut = np.zeros(self.LUTbuffer_len*2), np.zeros(self.LUTbuffer_len*2)
		self.I_lut[0::4] = self.I_dac[1::2] 		
		self.I_lut[1::4] = self.I_dac[0::2]
	...
	# Peak to peak should be 900 mV (from DAC)
		...
		plt.grid()
		plot1 = figure1.add_subplot(212)
		line2, = plot1.plot(np.arange(0,2048), np.zeros(2048), 'r-')
		plt.title('Q')
		plt.xlim(0,100)
		plt.ylim(-1.1,1.1)
		#plt.yticks(np.arange(-4e4, 4e4, 5000.))
	...
	def vna_sweep(self, center_freq = 750.0e6, save_path = '/mnt/iqstream/vna_sweeps', write = True):
		...
		save_path = os.path.join(save_path, sweep_dir)
		bb_freqs, delta_f = np.linspace(-255.5e6, 255.5e6, 1000,retstep=True)
		bb_freqs = np.roll(bb_freqs, - np.argmin(np.abs(bb_freqs)) - 1)
		np.save('/mnt/iqstream/last_bb_freqs.npy',bb_freqs)
		rf_freqs = bb_freqs + center_freq
		np.save('/mnt/iqstream/last_rf_freqs.npy',rf_freqs)
	...
	def store_UDP(self, Npackets, LO_freq, save_path, skip_packets=2, channels = None):
		...
		I_buffer,Q_buffer = self.get_UDP(self, Npackets, LO_freq, save_path, skip_packets=skip_packets, channels = channels)
	...
	def stream_UDP_dirfile(self,save_path=None,bufsize = int(244*0.2)):
		#open files
		"""
		...
		#write to files
		buf=np.empty(len(channels)*2*bufsize,dtype='int32')
		try:
			#counter=0  
			while(1):
				for i in range(bufsize):
					packet=self.s.recv(8192)
					data=np.fromstring(packet,dtype = '<i')
					buf[i::bufsize]=data
				for i in range(len(channels)*2):
	...
	def plot_kids(self, save_path = None, bb_freqs = None, channels = None):
		...
		[ plt.plot((sweep_freqs + bb_freqs[chan])/1.0e9,10*np.log10(np.sqrt(Is[:,chan]**2+Qs[:,chan]**2))) for chan in channels]
	...
	def plot_kids_dirfile(self, save_path = None,  channels = None):
		f,i,q = self.open_stored_dirfile(save_path)
		plt.figure('sweeps')
		[ plt.plot(f[chan]/1.0e9, 10*np.log10(np.sqrt(i[chan]**2+q[chan]**2))) for chan in channels]
		plt.xlabel('Frequency (GHz)')
		plt.ylabel('10log10 (S21 mag) [dB]')
		plt.title('250 um sweep')
		plt.savefig(os.path.join(save_path,'fig.png'))
		plt.show()
		return
